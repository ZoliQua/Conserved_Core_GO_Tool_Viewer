...
function loadSvg() {
    const svgFile = "data/ortholog_venn_7d.svg";
...
function calculateOverlaps(data) {
    // Initialize counts for each possible combination
    ...
    lines.forEach((line, index) => {
        ...
        const hasData = columns.slice(7, 14).map(col => col.trim() !== ''); // Adjust indices according to your file structure

        // Check each combination
        combinations.forEach(comb => {
            if (isCombinationPresent(comb, hasData)) {
                overlaps[comb]++;
            }
        });
    });
...
// Check if the combination is present in the data
function isCombinationPresent(combination, dataFlags) {
    for (let i = 0; i < combination.length; i++) {
        const index = combination.charCodeAt(i) - 65; // 'A' -> 0, 'B' -> 1, etc.
        if (!dataFlags[index]) return false;
    }
    return true;
}
